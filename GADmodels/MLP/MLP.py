...
def get_evaluate(model, feats, labels, settype, params, dataset_name, os_name, repeat):
...
if __name__ == '__main__':
    data = pd.read_csv('ListedCompanyFeatures_AP_missvalue.csv')  # 传入不同的缺失值处理后的文件

    split_dicts = {0: {'train': [2014,2015,2016], 'valid': [2017], 'test': [2018]},
                   1: {'train': [2014,2015,2016,2017], 'valid': [2018], 'test': [2019]},
                   2: {'train': [2014,2015,2016,2017,2018], 'valid': [2019], 'test': [2020]},
                   3: {'train': [2014,2015,2016,2017,2018,2019], 'valid': [2020], 'test': [2021]},
                   4: {'train': [2014,2015,2016,2017,2018,2019,2020], 'valid': [2021], 'test': [2022]}}
    ...
    for repeat in [0]:
        ...
        (train_x, train_y), (valid_x, valid_y), (test_x, test_y) = load_data(data, split_year)
        ...
        eval_list = [(train_x, train_y, 'train'),
                     (valid_x, valid_y, 'valid'),
                     (test_x, test_y, 'test')]
        for feats, labels, settype in eval_list:
            result = get_evaluate(model=clf,
                                  feats=feats, labels=labels, settype=settype,
                                  params=str(params), dataset_name=None,
                                  os_name=None, repeat=repeat)
